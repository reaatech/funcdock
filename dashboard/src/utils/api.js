...
// Request interceptor: auth is handled via httpOnly cookie automatically
...
export const functionsApi = {
  ...
  getFileContent: (name, filePath) =>
    api.get(`/api/functions/${name}/files/content?path=${encodeURIComponent(filePath)}`),

  // Download file
  downloadFile: (name, filePath) =>
    api.get(`/api/functions/${name}/files/download?path=${encodeURIComponent(filePath)}`, {
      responseType: 'blob',
    }),
...
export const layersApi = {
  ...
  getLayerFileContent: (name, filePath) =>
    api.get(`/api/layers/${name}/files/content?path=${encodeURIComponent(filePath)}`),

  // Download layer file
  downloadLayerFile: (name, filePath) =>
    api.get(`/api/layers/${name}/files/download?path=${encodeURIComponent(filePath)}`, {
      responseType: 'blob',
    }),
  ...
  // Update layer file
  updateLayerFile: (name, filePath, content) =>
    api.put(`/api/layers/${name}/files`, { path: filePath, content }),
